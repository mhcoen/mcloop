...
def main():
    ...
    # Not whitelisted — notify via Telegram and return "ask" so Claude Code
    # shows a permission prompt (approvable via Remote Control from phone)
    ...
    try:
        send_message(msg)
        _dbg("EXIT: notified, returning ask")
    except Exception as e:
        _dbg(f"EXIT: telegram send failed ({e}), returning ask anyway")
