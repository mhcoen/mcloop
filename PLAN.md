...
- [ ] Fix Ctrl-C: prevent claude from stealing the terminal foreground
  ...
  - [ ] [USER] Manual verification: user tests Ctrl-C, Ctrl-Z, and `kill <pid>` on a live mcloop run. Verify all three interrupt mcloop cleanly and kill the claude subprocess group. Verify stream-json output is still captured correctly in logs.
