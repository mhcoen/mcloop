...
- [ ] Fix Ctrl-C: run claude -p inside a pty
  ...
  - [ ] Manual verification: start mcloop on any project with unchecked tasks. Wait until a task session is running (progress dots visible). Press Ctrl-C. Confirm mcloop exits with "Interrupted." or note if it keeps running and ignores the keystroke. Repeat with Ctrl-Z. Then start mcloop again and from another terminal send `kill <mcloop-pid>`. Record results here when verified.
