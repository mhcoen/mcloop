...
- [ ] Clearer terminal output: suppress individual tool calls (Read, Edit, Write, Glob, Grep, TodoWrite) entirely. Only print Bash commands. Add a progress indicator (a dot every few seconds) while a claude -p session is running so it's clear mcloop is alive. Print a clear phase announcement when starting each activity (task, checks, audit, verify, fix) and a completion line with elapsed time when it finishes. Example: ">>> [TASK 13.2] Extracting frames..." then dots, then ">>> [TASK 13.2] Complete [1m 5s]". Keep Bash commands visible since they show meaningful actions.
